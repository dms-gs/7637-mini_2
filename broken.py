...
#            working_stack, stackable = strip_singles(bottoms, stackable)
            working_stack, stackable = self.strip_singles(unmet_goals, stackable)
...
#            print(f"solve, ja mark_bad: Stacks with bad marked: {marked_bad}")
            stackable = self.strip_bad(marked_bad, goal, stackable)
            for stack in stackable:  # check single block on table for target available
#                print("checking left overs")
                if len(stack) == 1:
                    target_block = self.find_target(stack[0], goal, stackable)
#                    print(f"{target_block = }")
                    stackable = self.move_block(stack[0], target_block, stackable)
                    stackable.remove(stack)
#            print(f"{stackable = }")
#            print(f"{move_list = }")
#            input()
...
#                print(f"target block to stack onto is {target_block}")
    ...
    def move_block(self, current_block, target_block, stackable):
    ...
    def strip_bad(self, marked_bad, goal, stackable):
        """Remove bad and eit"""
...
#                print(f"{current_block = }")
                if i == 1 and stack[1] == -1:
                    stackable.append(current_block)
                    continue
                target_block = self.find_target(current_block, goal, stackable)
                stackable = self.move_block(current_block, target_block, stackable)
...
#        print(f"strip_bad, jb exit: {marked_bad = }")
    ...
    def strip_singles(self, block_stacks, stackable):
...
#                    print(f"{good_to = }")
    ...
    #    input(f"{bad_bottoms = } {bad_no_bottoms = }")
        stacks_to_remove = []
