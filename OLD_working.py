...
#            working_stack, stackable = strip_singles(bottoms, stackable)
            working_stack, stackable = strip_singles(unmet_goals, stackable)
        ...
        # check next block up on bottoms
            ...
            for stack in stackable:  # check single block on table for target available
                ...
                if len(stack) == 1:
                    target_block = find_target(stack[0], goal, stackable)
                    print(f"{target_block = }")
                    stackable = move_block(stack[0], target_block, stackable)
...
def find_target(current_block, goal, stackable):
    ...
    for stack in stackable:
        if stack[-1] == target_block:
            target_target = find_target(target_block, goal, stackable)
